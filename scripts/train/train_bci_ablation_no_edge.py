#!/usr/bin/env python3
"""BCI ablation: no edge encoder. Identical to train_bci.py except edge_encoder=None, he_edge_weight=0."""
...
def main():
    ...
    parser.add_argument('--ckpt_dir', type=str, default='checkpoints/bci_ablation_no_edge')
    parser.add_argument('--batch_size', type=int, default=16)
    parser.add_argument('--max_epochs', type=int, default=100)
    parser.add_argument('--wandb_name', type=str, default='bci_ablation_no_edge')
    args = parser.parse_args()

    model = UNIStainNetTrainer(
        num_classes=5, null_class=4, class_dim=64, uni_dim=1024, ndf=64,
        input_skip=True,
        edge_encoder=None,          # ABLATION: no edge encoder
        edge_base_ch=32,
        uni_spatial_size=32,
        label_names=['0', '1+', '2+', '3+'],
        gen_lr=1e-4, disc_lr=4e-4, warmup_steps=1000,
        lpips_weight=1.0, lpips_256_weight=0.5, lpips_512_weight=0.0,
        he_edge_weight=0.0,         # ABLATION: no edge loss
        ...
        ema_decay=0.999,
        extract_uni_on_the_fly=True, uni_spatial_pool_size=32,
    )
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ModelCheckpoint(dirpath=args.ckpt_dir, filename='no_edge_{epoch:03d}_{step:06d}',
