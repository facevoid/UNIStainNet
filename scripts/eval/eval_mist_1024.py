#!/usr/bin/env python3
"""
Evaluate UNIStainNet on MIST dataset at native 1024x1024 resolution.

Same as eval_mist.py but loads data at 1024 and generates at 1024.

Usage:
    python scripts/eval/eval_mist_1024.py \
        --checkpoint checkpoints/mist_1024/last.ckpt \
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate UNIStainNet on MIST (1024)')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f'eval_output/mist_1024/{ckpt_name}'
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print(f"EVALUATION: UNIStainNet on MIST — 1024x1024")
